...
%PYTHON_PATH% -m pip install -r requirements%VENV_NAME_MAIN%.txt || (echo Error while installing packages for %VENV_NAME_MAIN% & exit /b %errorlevel%)

call conda deactivate

call conda activate %VENV_NAME_TRNSYS%
%PYTHON_PATH% -m pip install -r requirements%VENV_NAME_TRNSYS%.txt || (echo Error while installing packages for %VENV_NAME_TRNSYS%  & exit /b %errorlevel%)
